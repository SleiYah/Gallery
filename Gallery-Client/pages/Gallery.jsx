...
const Gallery = () => {
  ...
  const fetchPhotos = async () => {
    try {
      const response = await axios.post('http://localhost:80/projects/Gallery/gallery-server/getPhotos', {
        user_id: user
      });
      ...
      if (response.data.status === 'success') {
        ...
        // Extract unique tags (using the reduce method for clarity)
        const uniqueTags = response.data.photos.reduce((tags, photo) => {
          // Only add the tag if it exists and isn't already in our array
  ...
  const handleDeletePhoto = async (photoId) => {
    try {
      const response = await axios.post('http://localhost:80/projects/Gallery/gallery-server/deletePhoto', {
        user_id: user,
        photo_id: photoId
      });
    ...
    <div className="gallery-container">
      <header className="gallery-header">
        <div className="header-content">
          ...
          <div className="user-actions">
            <Link to="/add-photo" className="add-photo-btn">Add Photo</Link>
      ...
      {filteredPhotos.length === 0 ? (
        <div className="empty-gallery">
          ...
          <Link to="/add-photo" className="add-photo-btn">Add Your First Photo</Link>
        </div>
      ) : (
